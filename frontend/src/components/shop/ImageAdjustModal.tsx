/**
 * ImageAdjustModal — lets the shop owner frame an uploaded cover photo before
 * committing it. The picture is shown inside a fixed-aspect window that the user
 * can drag (pan) and zoom; on confirm we crop to that window with a canvas and
 * return a JPEG Blob. No external cropping library.
 */
...
interface ImageAdjustModalProps {
  file: File;
  /** Output aspect ratio width/height (cover hero ≈ 16/9). */
  aspect?: number;
  /** Rendered output width in px (height derived from aspect). */
  outputWidth?: number;
  busy?: boolean;
  onCancel: () => void;
...
export default function ImageAdjustModal({
  ...
  // Load the chosen file into an <img> to read its natural dimensions.
  useEffect(() => {
  ...
  // Measure the frame once it is in the DOM (and on resize).
  useEffect(() => {
    const measure = () => {
  ...
  // "cover" base scale so the image always fills the frame at zoom = 1.
  ...
  // Centre the image on first ready; just re-clamp on later zoom/geometry changes.
  ...
  const handleConfirm = () => {
    ...
    if (!ctx) return;
    // Map the frame viewport back into the source image's natural pixels.
    ...
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm animate-fade-in font-sans">
      <div className="w-full max-w-lg rounded-3xl bg-white p-5 shadow-2xl">
        <div className="mb-3 flex items-center justify-between">
          <h3 className="font-display text-base font-bold text-[#17140F]">Căn chỉnh ảnh bìa</h3>
        ...
        <p className="mb-3 inline-flex items-center gap-1.5 text-[11px] text-gray-500">
          <Move className="h-3.5 w-3.5 text-[#B8924A]" /> Kéo để di chuyển · dùng thanh trượt để phóng to
        </p>
        ...
        <div className="mt-4 flex items-center gap-3">
          ...
          />
          <span className="w-10 text-right text-[11px] tabular-nums text-gray-500">{zoom.toFixed(1)}×</span>
        </div>
        ...
        <div className="mt-5 flex gap-2 text-xs font-bold">
          ...
          >
            <Check className="h-4 w-4" /> {busy ? 'Đang tải lên…' : 'Chốt & dùng ảnh này'}
          </button>
          <button
            onClick={onCancel}
            disabled={busy}
